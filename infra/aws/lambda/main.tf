...
resource "aws_lambda_function" "update_priority_fn" {
  function_name = var.update_priority_function_name
  role          = aws_iam_role.resource_request_lambda_role.arn
  package_type  = "Image"
  image_uri     = "${var.update_priority_image_uri}:latest"
  timeout       = 15
  environment {
    variables = {
      DB_HOST             = var.db_host
...
resource "aws_lambda_event_source_mapping" "prioritization_sqs" {
  event_source_arn = var.prioritization_event_queue_arn
  function_name    = aws_lambda_function.update_priority_fn.arn
  batch_size       = 10
}
